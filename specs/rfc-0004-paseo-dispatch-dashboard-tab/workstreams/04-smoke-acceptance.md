...
- Validation run:
  ...
  - Reloaded an existing persisted `dispatch-dashboard` Web browser tab and
    confirmed the old browser session recovers into the dashboard iframe
    instead of the `Browser is desktop-only` fallback.
...
- Behavior/spec changes:
  ...
  - Web browser pane now subscribes to browser-store hydration and maps legacy
    `https:///dispatch-dashboard/...` values, generated by the previous bundle,
    back to same-origin dashboard paths.
